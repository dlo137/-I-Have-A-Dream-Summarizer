...
def score_sentences(speech, word_freq, max_words):
    ...
    sent_scores = dict()
    # tokenizing speech by sentences
    sentences = nltk.sent_tokenize(speech)
    # loop sentences to calculate scores and enforce max words per sentence
    for sent in sentences:
        words = nltk.word_tokenize(sent.lower())
        if len(words) <= int(max_words):
            sent_scores[sent] = sum(word_freq[word] for word in words if word in word_freq) / len(words)
    return sent_scores
